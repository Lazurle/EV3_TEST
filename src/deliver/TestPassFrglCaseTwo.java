...
class TestPassFrglCaseTwo{
	...
	private void passFragile() {
		...
		if(flag){
			...
			System.out.println("Test cleared "+test+" time(s).");
		}
	}
	...
	//コントロールメソッド
	//control method
	//受取人が不在の時、正しく動作するかをテストする。具体的には、passFragile()メソッドの最初の通信時に応答が
	//なければ、例外を出した後、受取人不在を記録してテストをクリアする。
	//test whether the system works properly when house absent.
	//see passFragile().  When first communication fail, catch exception then, record
	//absent
	public void control(){
		//int test=0;
