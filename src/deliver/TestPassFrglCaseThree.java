...
class TestPassFrglCaseThree{
	...
	private String adjust(Adjustment order) {
		...
		if(order==Adjustment.relayProtocol){
			...
			//テストケース使用
			//受取人宅の住所を異なる住所に変えて、宛先間違いと見なせるかをテストする
			//made up for test case
			//setting wrong address for House, test whether the system can judge wrong house
			detail="personAddrName|Ren Sato|0-0|Yuduki Suzuki|0-2";
	...
	//コントロールメソッド
	//control method
	//テスト内容はadjust(Adjustment order)メソッドを参照
	//test detail is written in adjust(Adjustment order).
	public void control(){
		int test=0;
		...
		while(this.hasFragile){
			...
			//宛先が間違っていれば分岐を通り、正しく実行できていることを報告する
			//when wrong house, report to execute correctly
