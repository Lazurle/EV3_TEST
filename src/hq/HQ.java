...
public class HQ {
	...
	private Boolean telecomunicationSendToReception(String SendDetail) {
		while (true) {
			try {
				if (this.boundaryHouse.sendSignal("本部から受付所へ。。", Receiver.house, Receiver.hq, 4)) {//送信成功
					System.out.printf("本部から受付所へ" + SendDetail + "送信成功\n");
					return true;
				} else {//送信失敗
					System.out.printf("送信失敗\n");
					return false;
				}
			} catch (IOException e) {//インターバルが必要
				System.out.printf(e.getLocalizedMessage() + "\n");
	...
	private Boolean telecommunicationSendToHouse(String SendDetail) {
		while (true) {
			try {
				if (this.boundaryHouse.sendSignal("本部から受取人宅へ。", Receiver.house, Receiver.hq, 4)) {//送信成功
					System.out.printf("本部から受取人宅へ" + SendDetail + "送信成功\n");
					return true;
				} else {//送信失敗
					System.out.printf("送信失敗\n");
					return false;
